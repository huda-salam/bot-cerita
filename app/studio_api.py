...
async def post_project(request: ProjectCreate):
    return create_project(request.title, request.universe_id)

@router.get("/studio/projects/{project_id}")
async def get_project_endpoint(project_id: str):
    project = get_project(project_id)
    if not project: raise HTTPException(status_code=404, detail="Project not found")
    project["scenes"] = list_scenes(project_id)
    return project
...
async def post_panel(scene_id: str, request: PanelCreate):
    return add_panel(scene_id, request.panel_number, **request.model_dump(exclude={"panel_number"}))
