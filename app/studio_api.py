...
from .studio_persistence import create_project, get_project, update_project, add_scene, list_scenes, add_panel, get_panel, get_scene, set_panel_references, list_panel_references
...
from .reference_pipeline import select_panel_references
...
@router.post("/studio/panels/{panel_id}/references/auto")
async def auto_panel_references(panel_id: str):
    panel=get_panel(panel_id)
    if not panel: raise HTTPException(status_code=404, detail="Panel not found")
    scene=get_scene(panel["scene_id"])
    if not scene: raise HTTPException(status_code=404, detail="Scene not found")
    character_ids=list_project_characters(scene["project_id"])
    assets={cid:list_character_assets(cid) for cid in character_ids}
    selected=select_panel_references(panel, assets, character_ids)
    saved=set_panel_references(panel_id, selected)
    return {"panel_id":panel_id,"references":saved,"count":len(saved),"strategy":"panel-aware canonical ranking"}
